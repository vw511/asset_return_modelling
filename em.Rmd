...
EM_mix_trunc_normal <- function(Y, K, pi0, mu0, Sigma0, Max_iter){
  
  # epsilon - the iteration ends when the improvement of MLE is less than epsilon
  ...
  # print(c("E_step's mu and sigma:", E_step$mu, E_step$Sigma))
  M_step <- M_Gaussian(Y, K, E_step$gamma, E_step$mu, E_step$Sigma)
  mu <- M_step$mu
  Sigma <- M_step$Sigma
  pi <- M_step$pi
  current_loglik <- E_step$Qold
  # print(c('initial conditional expectation of ll:', current_loglik))
  
  for (iter in 1: Max_iter) {
    E_step <- E_Gaussian(Y, K, M_step$mu, M_step$Sigma, M_step$pi)
    M_step <- M_Gaussian(Y, K, E_step$gamma, E_step$mu, E_step$Sigma)
    mu <- M_step$mu
    Sigma <- M_step$Sigma
    pi <- M_step$pi
    ...
    err <- abs((current_loglik - E_step$Qold))
    # print(err)
...
EM_estimate2 <- EM_mix_trunc_normal(Y, K=3, pi0, mu0, Sigma0, 5)
...
Sigma0 <- c(1, 1, 1)
pi0 <- c(0.8, 0.1, 0.1)
EM_estimate <- EM_mix_trunc_normal(Y, K=3, pi0, mu0, Sigma0, 7)
...
Sigma0 <- c(1, 1, 1)
pi0 <- c(0.8, 0.1, 0.1)
EM_estimate <- EM_mix_trunc_normal(Y, K=3, pi0, mu0, Sigma0, 9)
...
Sigma0 <- c(1, 0.5, 0.5)
pi0 <- c(0.8, 0.1, 0.1)
EM_estimate <- EM_mix_trunc_normal(Y, K=3, pi0, mu0, Sigma0, 4)
